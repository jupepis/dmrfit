...
// FisherMALA Pseudoposterior log acceptance ratio
double fishermala_pseudo_log_acceptance_ratio(const arma::vec& current_pars,
                                        const arma::vec& proposed_pars,
                                        const arma::vec& pseudo_gradient_current,
                                        const arma::vec& pseudo_gradient_proposed,
                                        const arma::mat& R_n,
    ...
    arma::vec step_current = current_pars - proposed_pars - ((sigma2_R / 4.0) * (R_n * (R_n.t() * pseudo_gradient_proposed)));
    double h_current_pars = arma::dot(step_current, pseudo_gradient_proposed);
    arma::vec step_proposed = proposed_pars - current_pars - ((sigma2_R / 4.0) * (R_n * (R_n.t() * pseudo_gradient_current)));
    double h_proposed_pars = arma::dot(step_proposed, pseudo_gradient_current);
...
// [[Rcpp::export]]
Rcpp::List cpp_pseudo_sampler(const arma::mat &data,
                                const arma::vec &pars, 
    ...
    // --- Calculate pseudo gradient and normalizing constant at current parameters ---
    PseudoGradient pseudo_current = compute_pseudo_gradient(obs_stats, unique_data, frequency, current_pars, current_pars,
    ...
    while(s < nsim){
        ...
        arma::vec proposed_pars = fishermala_propose(current_pars, pseudo_current.gradient, R_n, sigma2_R);

        // (2) Pseudo gradient and logZ at new parameters
        PseudoGradient pseudo_proposed = compute_pseudo_gradient(obs_stats, unique_data, frequency, proposed_pars, current_pars,
                                                                    n_categories, which_stats, lower_indices, category_offsets, P, N_unique, H,
                                                                    n_pars, n_thresholds, thresholds_alpha, thresholds_beta,
                                                                    interactions_location, interactions_scale);

        // (3) Compute acceptance ratio
        double log_a = fishermala_pseudo_log_acceptance_ratio(current_pars, proposed_pars, pseudo_current.gradient, pseudo_proposed.gradient,
                                                                R_n, obs_stats, pseudo_proposed.logZ_ratio, sigma2_R, n_thresholds, n_pars, 
                                                                thresholds_alpha, thresholds_beta, interactions_location, interactions_scale);

        // (4)  Adaptive step for R_n (FisherMALA)
        fishermala_update_preconditioner(R_n, log_a, pseudo_current.gradient, pseudo_proposed.gradient, I_d, lambda, s, adaptive_stage_n_iter);
        ...
        if(u < std::exp(log_a)){
            ...
            pseudo_current.gradient             = pseudo_proposed.gradient;
            if(s >= past_burnin){
