...
Rcpp::List dmrf_deriv(
    ...
    bool with_prior, // whether to include (TRUE) or not (FALSE) prior information (non-informative priors are used)
    int ncores)
{
    ...
    if(with_prior){
        ...
        loglik -= (arma::accu(log_beta_prime(thresholds)) + arma::accu(log_dcauchy(interactions_vec)));
        // prior on gradient
        arma::vec log_prior_gradient_thresholds = log_beta_prime_first_derivative(thresholds);
        arma::vec log_prior_gradient_interactions = log_dcauchy_first_derivative(interactions_vec);
        ...
        arma::vec log_prior_hessian_thresholds = log_beta_prime_second_derivative(thresholds);
        arma::vec log_prior_hessian_interactions = log_dcauchy_second_derivative(interactions_vec);
